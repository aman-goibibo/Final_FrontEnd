...
class CheckBoxImage extends Component {
  ...
  handleClick(link, tag) {
    if (
      typeof this.state.selected_urls[
        this.state.selected_urls.findIndex(x => x.url === link)
      ] === "undefined"
    ) {
      this.state.selected_urls.push({ url: link });
        ...
        var obj = { url : link}
        this.state.selected_urls = this.state.selected_urls.filter(function( obj ) {
            return obj.url !== link;
          });
    }
    console.log(this.state.selected_urls)
  }
  ...
  display() {
    ...
    return this.props.urls.map(url => {
      ...
      return (
        <li>
          <input type="checkbox" id={id_var} />
          <label for={id_var}>
            <img
              src={url.url}
              onClick={() => this.handleClick(url.url, url.tag)}
              alt=""
            />
          </label>
        </li>
...
export default compose(graphql(addStoryQuery, { name: "addStoryQuery" }))(
  CheckBoxImage
);
